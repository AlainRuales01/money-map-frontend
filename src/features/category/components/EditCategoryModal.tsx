...
import { useAddCategoryMutation, useCategoriesModifyInfoQuery } from '../hooks/useCategoryHooks';
...
const EditCategoryModal = ({id, onClose}: EditCategoryModalProps) => {
    ...
    const { data: categoryData, isError: isCategoryError, error: categoryError } = useCategoriesModifyInfoQuery({ id });

    const [name, setName] = useState(categoryData?.name || "");
    const [description, setDescription] = useState(categoryData?.description || "");
    const [categoryTypeId, setCategoryTypeId] = useState(categoryData?.categoryTypeId || "");
    ...
    }, [isCategoryError, categoryError, showAlert]);
